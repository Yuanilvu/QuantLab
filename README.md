...
- **6 track · 27 bab · 270 skenario keputusan · 43 soal coding · 27 Lab Teknikal**
...
- **📈 Lab Teknikal (27 drill, 4 modul)** — latihan analisa teknikal pakai grafik harga NYATA
  (BBRI, TLKM, IHSG, BRPT, CUAN, ALII, BTC): baca candle & tren → level & struktur →
  indikator & risiko → **praktik klik langsung di grafik** (tandai support, pasang stop loss,
  tentukan entry/TP — dinilai dengan toleransi zona). Grafik SVG server-side (line &
  candlestick + volume + RSI), data dibekukan di `data/chart/*.csv` supaya kunci jawaban
  tidak pernah basi (dijaga `scripts/verify_charts.py`). Halaman `/chart`, XP 20/35/50.
